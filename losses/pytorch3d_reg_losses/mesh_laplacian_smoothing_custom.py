...
'''
Code from pytorch3d: https://pytorch3d.readthedocs.io/en/latest/_modules/pytorch3d/loss/mesh_laplacian_smoothing.html
'''
...
def mesh_laplacian_smoothing(meshes, method: str = "uniform"):
    ...
    output_ = torch.cat([torch.sum(loss[verts_packed_idx==i]).reshape((1,1)) for i in range(N)], dim=0)
    return output_